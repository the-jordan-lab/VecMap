...
except ImportError:  # Fallback for standalone runs
    from vecmap.core.mapper import generate_reference, generate_reads

    def generate_transcriptome(num_transcripts):
        """Simplified transcriptome generator returning concatenated sequence."""
        ref_len = num_transcripts * 1000
        ref = generate_reference(ref_len)
        transcript_info = [(i * 1000, (i + 1) * 1000) for i in range(num_transcripts)]
        position_map = {i: start for i, (start, _) in enumerate(transcript_info)}
        return ref, transcript_info, position_map

    def simulate_rnaseq_reads(ref, position_map, num_reads, read_len=100):
        """Simplified RNA-seq read simulation."""
        reads = []
        for _ in range(num_reads):
            start = np.random.randint(0, len(ref) - read_len)
            seq = ref[start : start + read_len]
            reads.append((seq, start))
        return reads
        ...
        try:
            if self.version_cmd:
                result = subprocess.run(
                    self.version_cmd, shell=True, capture_output=True, text=True
                )
...
class VecMapTool(BenchmarkTool):
    ...
    def __init__(self, use_numba: bool = False):
        name = "VecMap-Numba" if use_numba else "VecMap"
    ...
    def align_direct(self, ref_sequence, reads, read_length=100):
        ...
        if self.use_numba:
            from vecmap import vecmap_numba as mapper
        else:
            from vecmap import vecmap as mapper
        mappings = mapper(ref_sequence, reads, read_length)
        end_time = time.time()
...
class KallistoTool(BenchmarkTool):
    ...
    def install(self):
        ...
        subprocess.run(
            ["conda", "install", "-y", "-c", "bioconda", "kallisto"], check=True
        )
        self.check_installation()
...
class SalmonTool(BenchmarkTool):
    ...
    def install(self):
        ...
        subprocess.run(
            ["conda", "install", "-y", "-c", "bioconda", "salmon"], check=True
        )
        self.check_installation()
...
class MiniMap2Tool(BenchmarkTool):
    ...
    def install(self):
        ...
        subprocess.run(
            ["conda", "install", "-y", "-c", "bioconda", "minimap2"], check=True
        )
        self.check_installation()
...
def generate_test_data(num_transcripts=100, num_reads=10000):
    ...
    ref_sequence, transcript_info, position_map = generate_transcriptome(
        num_transcripts
    )
...
def run_vecmap_benchmark(ref_sequence, reads, use_numba: bool = False):
    """Run VecMap benchmark"""
    impl = "VecMap (Numba)" if use_numba else "VecMap"
    print(f"Running {impl}...")
    tool = VecMapTool(use_numba=use_numba)
...
def plot_results(results_df):
    ...
    # Accuracy (for VecMap)
    vecmap_results = results_df[results_df["tool"].str.startswith("VecMap")]
    if not vecmap_results.empty:
        metrics = ["Mapping Rate", "Accuracy"]
        values = [
            vecmap_results["mapping_rate"].iloc[0],
...
def create_summary_table(results_df):
    ...
    summary = results_df[
        ["tool", "version", "time_mean", "reads_per_second", "memory_mb"]
    ].copy()

    # Add relative performance
    vecmap_tool = results_df[results_df["tool"].str.startswith("VecMap")]
    vecmap_speed = vecmap_tool["reads_per_second"].iloc[0]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Benchmark VecMap against other tools")
    parser.add_argument(
        "--vecmap-impl",
        choices=["python", "numba"],
        default="python",
        help="VecMap implementation to benchmark",
    )
    ...
    for config in test_configs:
        ...
        vecmap_results = run_vecmap_benchmark(
            ref_sequence, reads, use_numba=args.vecmap_impl == "numba"
        )
    ...
    plot_results(
        results_df[
            results_df["config"]
            == f"{test_configs[0]['transcripts']}tx_{test_configs[0]['reads']}r"
        ]
    )
    ...
    baseline_tool = "VecMap-Numba" if args.vecmap_impl == "numba" else "VecMap"
    vecmap_data = results_df[results_df["tool"] == baseline_tool]
    if len(vecmap_data) > 0:
        print(f"\nVecMap Performance:")
        print(
            f"  Average speed: {vecmap_data['reads_per_second'].mean():.0f} reads/second"
        )
        ...
        if len(other_tools) > 0:
            ...
            print(f"\nComparison:")
            print(
                f"  VecMap is {vecmap_data['reads_per_second'].mean() / avg_other_speed:.1f}x the average speed of other tools"
            )
            print(
                f"  VecMap uses {vecmap_data['memory_mb'].mean() / other_tools['memory_mb'].mean():.1%} of the memory"
            )
